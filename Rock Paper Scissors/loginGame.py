...
# PostgreSQL डेटाबेस से कनेक्ट करने के लिए फ़ंक्शन (Function to connect to PostgreSQL database)
...
# यह फ़ंक्शन गेम में जीतने-हारने-टाई होने के पूरे लॉजिक (तर्क) को संभालता है। (This function handles the whole win-lose-tie logic in the game.)
def youPick(yourChoice):
    ...
    # Here we decide the result based on user and computer choices
    # यहाँ हम यूज़र और कंप्यूटर की पसंद के आधार पर परिणाम तय करते हैं (Here we determine the outcome based on user and computer choice)
    ...
    if counter == turns:
        ...
        confetiImg = PhotoImage(file=os.path.join(BASE_DIR, "Images", "confeti.gif"))
...
def start():
    ...
    # ----------------Image set----------------
    rHandPhoto = PhotoImage(file=os.path.join(BASE_DIR, 'Images', 'rHand.png'))
    pHandPhoto = PhotoImage(file=os.path.join(BASE_DIR, 'Images', 'pHand.png'))
    sHandPhoto = PhotoImage(file=os.path.join(BASE_DIR, 'Images', 'sHand.png'))
    rock = Image.open(os.path.join(BASE_DIR, "Images", "Rockimg.jpg"))
    rockImage = ImageTk.PhotoImage(rock)
    paper = Image.open(os.path.join(BASE_DIR, "Images", "Paperimg.jpg"))
    paperImage = ImageTk.PhotoImage(paper)
    scissors = Image.open(os.path.join(BASE_DIR, "Images", "Scissorsimg.jpg"))
    scissorImage = ImageTk.PhotoImage(scissors)
    win = Image.open(os.path.join(BASE_DIR, "Images", "YouWin.jpg"))
    winImage = ImageTk.PhotoImage(win)
    lose = Image.open(os.path.join(BASE_DIR, "Images", "YouLose.jpg"))
    loseImage = ImageTk.PhotoImage(lose)
    tie = Image.open(os.path.join(BASE_DIR, "Images", "YouTie.jpg"))
...
def welcomeUserPage():
    ...
    if UserGender == "Male":
        maleimg = ImageTk.PhotoImage(Image.open(os.path.join(BASE_DIR, "Images", "Male.jpg")))
        maleLabel = Label(root ,image=maleimg)
        maleLabel.image = maleimg
        maleLabel.grid(row=1, column=0)
        compimg = PhotoImage(file=os.path.join(BASE_DIR, "Images", "computer.png"))
        compLabel = Label(root, image=compimg)
        compLabel.image = compimg
        compLabel.grid(row=1, column=1)
    else:
        femaleimg = ImageTk.PhotoImage(Image.open(os.path.join(BASE_DIR, "Images", "Female.jpg")))
...
# डेटाबेस में यूज़रनेम और पासवर्ड की जाँच करने के लिए (To check username and password in the database)
def check():
    global username, password, USER, UserGender
    conn = get_db_connection()
    c = conn.cursor()
    c.execute("SELECT * from USERS where username = %(user)s and password = %(pass)s",
              {
                  'user': username.get(),
                  'pass': password.get()
              }
...
# USERS टेबल बनाने के लिए फ़ंक्शन - शुरुआत में सिर्फ एक बार चलाया जाना चाहिए (Function to create USERS table - Should be run only once)
def create():
    conn = get_db_connection()
    c = conn.cursor()
    c.execute("""
...
# LEADERBOARD टेबल बनाने के लिए फ़ंक्शन (Function to create LEADERBOARD table)
...
# लीडरबोर्ड में डेटा डालने/अपडेट करने के लिए (To insert/update data in leaderboard)
def insertToLeaderBoard():
    conn = get_db_connection()
    c = conn.cursor()
    c.execute("SELECT * FROM LEADERBOARD WHERE firstname = %(USER)s", {
        'USER' : USER
    })
    data = c.fetchone()
    if data:
        ...
        c.execute("""
            ...
            GamesPlayed = %s,
            WiningRate = %s        
            WHERE firstname = %s;""",(won, lost, tied, played, rate, USER))
    conn.commit()
    conn.close()
    return
...
# लीडरबोर्ड बनाने के लिए फ़ंक्शन (Function to construct the Leader Board)
def getLeaderboard():
...
# GUI पर रिकॉर्ड प्रिंट करने के लिए (To print records on the GUI)
def printdata():
...
# USERS और LEADERBOARD डेटाबेस में नया रिकॉर्ड जोड़ने के लिए फ़ंक्शन (Function to insert data in USERS and LEADERBOARD database)
def insert():
    global fname, lname, username, password, gender
    conn1 = get_db_connection()
    c = conn1.cursor()
    c.execute("INSERT INTO USERS VALUES(%(fname)s, %(lname)s, %(username)s, %(password)s, %(gender)s)",
              {
                  'fname':fname.get(),
                  'lname':lname.get(),
    ...
    c2 = conn2.cursor()
    c2.execute("""
            INSERT INTO LEADERBOARD VALUES(%(name)s, 0, 0, 0, 0, 0)""",{'name' : fname.get()})
    conn2.commit()
    conn2.close()
    # clear the textboxes
    fname.delete(0,END)
